...
namespace PiggsCare.DataAccess.Repositories.Weaning
{
    public class WeaningRepository( ISqlDataAccess dataAccess, IDateConverterService dateConverterService ):IWeaningRepository
    {
        ...
        {
            IEnumerable<WeaningEventDto> result = await dataAccess.QueryAsync<WeaningEventDto, dynamic>("sp.Weaning_GetAll", new { FarrowingEventId = id });
            return result.Select(x => new WeaningEvent(
                                     x.WeaningEventId,
                                     x.FarrowingEventId,
        ...
        public async Task<WeaningEvent?> GetWeaningEventByIdAsync( int id )
        {
            ...
            IEnumerable<WeaningEventDto> result = await dataAccess.QueryAsync<WeaningEventDto, dynamic>("sp.Weaning_GetUnique", new { WeaningEventId = id });
            ...
            return weaningEventDto is not null
                ? new WeaningEvent(
                    weaningEventDto.WeaningEventId,
        ...
        public async Task<int> CreateWeaningEventAsync( WeaningEvent weaning )
        {
            ...
            IEnumerable<int> result = await dataAccess.QueryAsync<int, dynamic>(
                ...
                new
                {
                    record.FarrowingEventId,
                    record.WeaningDate,
                    record.NumberWeaned,
                    record.MalesWeaned,
                    record.FemalesWeaned,
        ...
        public async Task UpdateWeaningEventAsync( WeaningEvent weaning )
        {
            ...
            await dataAccess.CommandAsync("sp.Weaning_Modify", recordDto);
        }

        public async Task DeleteWeaningEventAsync( int id )
        {
            await dataAccess.CommandAsync("sp.Weaning_Delete", new { WeaningEventId = id });
