...
{
    public class FarrowRecordValidation:IFarrowRecordValidation, INotifyDataErrorInfo, INotifyPropertyChanged
    {
        #region Fields
        ...
        public void ValidateProp( object value, string propertyName = "" )
        {
            switch (propertyName)
            {
                case nameof(FarrowEvent.FarrowDate):
                    ValidateFarrowDate((DateOnly)value, propertyName);
                    break;
                case nameof(FarrowEvent.LitterSize):
                case nameof(FarrowEvent.BornAlive):
                case nameof(FarrowEvent.BordDead):
                case nameof(FarrowEvent.Mummified):
                    ValidateIntProperty((int)value, propertyName);
                    break;
            }
        }

        private void ValidateFarrowDate( DateOnly value, string propertyName )
        {
        ...
        private void ValidateIntProperty( int value, string propertyName = "" )
        {
            ...
            if (value < 1)
                ...
                AddError(propertyName, "Record is empty.");
        }
