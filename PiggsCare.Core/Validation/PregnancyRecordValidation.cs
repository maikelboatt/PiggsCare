...
{
    public class PregnancyRecordValidation:IPregnancyRecordValidation, INotifyDataErrorInfo, INotifyPropertyChanged
    {
        #region Fields
        ...
        public void ValidateProp( object value, string propertyName = "" )
        {
            switch (propertyName)
            {
                case nameof(PregnancyScan.ScanDate):
                    ValidateScanDate((DateOnly)value, propertyName);
                    break;
                case nameof(PregnancyScan.ScanResults):
                    ValidateStringProperty((string)value, propertyName, "Field cannot be empty", 3, 30);
                    break;
            }
        }

        private void ValidateScanDate( DateOnly value, string propertyName )
        {
